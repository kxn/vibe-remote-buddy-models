...
- 完整原始字节见 [当前指纹](../fingerprints/xiaomi/remote-2-pro-rc003.json)。
...
本轮没有重新逐键按压，也没有采集 PnP/Model Number；键码表仍明确标为 existing-builtin-mapping。不得把以上记录称为这些字段的实测验证。完整 Map 已补齐。旧格式指纹退出当前库，历史证据由 Git 保存。
